...
const items = computed(() => {
  if (isAdmin.value) {
    return [
      ...
      { to: '/admin/students', label: 'Étudiants', icon: 'bi-people' },
      { to: '/admin/subjects', label: 'Matières', icon: 'bi-journal' },
      { to: '/admin/grades', label: 'Notes', icon: 'bi-123' },
      { to: '/complaints', label: 'Réclamations', icon: 'bi-inbox' },
    ];
  }
  if (isTeacher.value) {
    return [
      { to: '/teacher', label: 'Dashboard', icon: 'bi-speedometer2' },
      { to: '/teacher/grades', label: 'Notes', icon: 'bi-123' },
      { to: '/teacher/complaints', label: 'Réclamations', icon: 'bi-inbox' },
    ];
  }
  return [
    { to: '/student', label: 'Dashboard', icon: 'bi-speedometer2' },
    { to: '/grades', label: 'Mes notes', icon: 'bi-mortarboard' },
    { to: '/complaints', label: 'Réclamations', icon: 'bi-inbox' },
  ];
});
</script>
